...
export function getLeadLikedConceptLabel(value?: string | null) {
  return LEAD_LIKED_CONCEPT_OPTIONS.find((option) => option.value === value)?.label ?? (value ?? "Unknown")
}

export function getLeadWouldRecommendLabel(value?: string | null) {
  return LEAD_WOULD_RECOMMEND_OPTIONS.find((option) => option.value === value)?.label ?? (value ?? "Unknown")
}

export function getLeadWillingToPayRangeLabel(value?: string | null) {
  return LEAD_WILLING_TO_PAY_RANGE_OPTIONS.find((option) => option.value === value)?.label ?? (value ?? "Unknown")
}
